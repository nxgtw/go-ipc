...
func testLockerOpenMode(t *testing.T, ctor lockerCtor, dtor lockerDtor) bool {
	...
	lk, err := ctor(testLockerName, O_READWRITE, 0666)
	a.Error(err)
	lk, err = ctor(testLockerName, O_CREATE_ONLY|O_READ_ONLY, 0666)
	a.Error(err)
	lk, err = ctor(testLockerName, O_OPEN_OR_CREATE|O_WRITE_ONLY, 0666)
	a.Error(err)
	lk, err = ctor(testLockerName, O_OPEN_ONLY|O_WRITE_ONLY, 0666)
	a.Error(err)
	lk, err = ctor(testLockerName, O_CREATE_ONLY, 0666)
	if !a.NoError(err) {
		return false
	}
	defer func(lk IPCLocker) {
		if d, ok := lk.(Destroyer); ok {
			a.NoError(d.Destroy())
		} else {
			a.NoError(lk.Close())
		}
	}(lk)
	lk, err = ctor(testLockerName, O_OPEN_ONLY, 0666)
	if !a.NoError(err) {
		return false
	}
	a.NoError(lk.Close())
...
func testLockerOpenMode3(t *testing.T, ctor lockerCtor, dtor lockerDtor) bool {
	...
	lk, err = ctor(testLockerName, O_OPEN_OR_CREATE, 0666)
	if !a.NoError(err) {
		return false
	}
	a.NoError(lk.Close())
